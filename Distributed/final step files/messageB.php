...
function send_message($message)
{
  ...
  $message = $message.'nodeB'; //ADD YOUR NAME HERE
  ...
  // START CHANGES
  // Build the data array and the POST request
  $data = array('message' => $message." sent to ".$next_nodeB);
  ...
  if ($response != false) {
	...
	if ($response != false) {
		...
		$response = array('status' => false, 'msg' => 'Failed at node: '.$next_nodeA, 'count' => 0);
		return $response;
	}
  }
  // END CHANGES
